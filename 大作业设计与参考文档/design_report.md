...
针对 EES351-MC 实验箱的 STM32 板载资源，我们进行了标准化的引脚规划：

| 外设模块 | 引脚名称 | STM32 管脚 | 接口模式 | 功能描述 |
| :--- | :--- | :--- | :--- | :--- |
| **AHT20 / BMP280** | SCL | `PB6` | 模拟 I2C 时钟 | 传感器 I2C 总线时钟线 |
| | SDA | `PB7` | 模拟 I2C 数据 | 传感器 I2C 总线数据线 |
| **TFT-LCD** | SCL/CLK | `PA5` | SPI 时钟 | LCD 串行时钟线 |
| | SDA/MOSI| `PA7` | SPI 主出从入 | LCD 数据传输线 |
| **TFT-LCD 辅助** | CS | `PA4` | GPIO 输出 | LCD 片选信号 (低电平有效) |
| | RS/DC | `PA6` | GPIO 输出 | 数据/命令选择引脚 (0=命令, 1=数据) |
| | RST | `PA3` | GPIO 输出 | LCD 复位引脚 |
| | BL / LED| `PB1` | PWM 输出 | 屏幕背光亮度控制 |
| **状态 LED** | LED1 | `PC13` | GPIO 输出 / 定时器 | 系统状态指示灯 1（板载，低电平有效） |
| | LED2 | `PA0` | TIM2_CH1 PWM | 系统状态指示灯 2（高电平有效，用于呼吸灯）|
| **独立按键** | KEY1 | `PB0` | GPIO 输入 / EXTI0 | 翻页/历史查询按键 (带消抖) |
| | KEY2 | `PB8` | GPIO 输入 / EXTI8 | 警报消音 / 学习参数重置按键 |
...
2. **学习阶段 (Learning Phase)**:
   系统在上电前 $M$ 次采集（例如前 3 分钟或 180 次采样）进入“自适应学习模式”。在此期间，系统只记录数据并动态更新历史均值（Baseline Mean）：
   $$\mu_{base}[n] = \frac{n-1}{n}\mu_{base}[n-1] + \frac{1}{n}T_f[n]$$
   此时 LED 呈缓速渐变呼吸状态。

3. **智能监测阶段 (Monitoring Phase)**:
   学习期结束后，锁定历史基准均值 $\mu_{base}$。此后，每次采集新值 $T_f$，计算其与基准的偏差：
   $$\Delta T = |T_f - \mu_{base}|$$
   - 若 $\Delta T \le 5^\circ\text{C}$，判定为 **正常 (NORMAL)**。
   - 若 $\Delta T > 5^\circ\text{C}$，判定为 **异常 (ANOMALY)**。此时状态机将 LED 切换至高频闪烁（Blinking）警报模式。
...
### 5.1 现场验收展示重点
1. **呼吸灯过渡到闪烁**：现场手部捂住传感器加热，当温度迅速升温偏离历史基准 5°C 时，观察 LED 立刻从温和的呼吸效果切换为刺眼的急促闪烁，同时 LCD 顶部弹出明显的红底白字 “ANOMALY DETECTED”。
