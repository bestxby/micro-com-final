...
| 红外接收 (LED1已禁用) | **PC13** | B5 | V18 | → | (N/A) | (N/A) |
...
| B5 | PC13 | 红外接收 (LED1已禁用) | D1 | PB7 | WiFi_RXD (重映射) |
...
|:---:|:---:|:---|:---:|:---|:---|
| **LED1** | **PC13** | (已禁用) | - | - | 告警指示灯已禁用，PC13 已被重映射为红外接收引脚 |
...
| 引脚 | 功能模块 | 信号角色 | 模式 | Arduino |
|:---:|:---|:---|:---|:---:|
| **PC13** | 红外接收器 | IR_DAT (红外接收) | 上拉输入 | B5 |
...
1. **传感器数据非阻塞采集**: 通过软件 I²C 总线周期性轮询 AHT20 (温湿度)、BMP280 (气压/温度) 和 BH1750 (光照度)，采样间隔约 1.5 秒。AHT20 采用非阻塞读取方案（启动测量与数据读取分离，中间依靠 200ms 的状态机轮询代替延时等待），彻底消除每次轮询带来的 80ms 页面刷新卡顿。
...
4. **实时异常检测与告警**: 滤波后温度偏离学习基准超过 ±5°C 时触发异常告警。告警状态直接在 LCD UI 上进行指示。
5. **六页 UI 交互与游戏**: 提供 6 页 UI 交互，包括 Page 0 实时监测数据、Page 1 AI 学习折线图、Page 2 历史异常日志、Page 3 经典 Flappy Bird 游戏等。在进行游戏时，系统会自动暂停 Wi-Fi 网络轮询与重连检测，确保游戏帧率稳定无卡顿。
6. **ESP8266 串口性能优化**: Wi-Fi 驱动的 `ESP8266_WaitReply()` 引入了 `\r\nOK\r\n` / `\r\nERROR\r\n` 等关键字提前终止机制，并将心跳查询超时从 800ms 缩短到 150ms，使得网络检测延迟从 800ms 降至 20ms 以内，彻底消除断网时的界面顿挫。
7. **SD卡自适应响应与防卡死**: 针对 SanDisk 16GB 等现代高速度 SDHC 卡，增加了 NCR 响应延迟等待轮询，解决原驱动直接单字节读取返回 `0xFF` 导致初始化失败的问题；同时限制 `CMD0` 重试次数为 3 次，并将低速下响应超时扩充为 512，避免未插卡时产生过长的系统卡死阻塞（总阻塞时间从数秒减少至 184ms 内）。
8. **红外遥控引脚重映射与多重按键映射**: 
   - **引脚重映射**：红外接收 DAT 引脚从有串口冲突的 `PA9` 迁移至 `PC13`，使用 `EXTI13` 双边沿中断配合 `TIM4` 硬件微秒级定时器完成高精度的 NEC 红外帧解码。PC13 原有的 `LED1` 输出逻辑被关闭。
   - **双重按键映射**：支持遥控器上**下方物理导航按键 (D-pad + OK)** 与**上方数字键 (1, 3, 5, 6, 7, 8, 9)** 的双重指令映射：
     - **左翻页**：物理左键 `◄` (`0x08`) 或 数字键 `7` (`0x07`)
     - **右翻页**：物理右键 `►` (`0x5A`) 或 数字键 `9` (`0x09`)
     - **布防开关**：物理 `OK` 键 (`0x1C`) 或 数字键 `1` (`0x45`) / `8` (`0x15`)
     - **游戏跳跃/开始**：物理上键 `▲` (`0x18`) 或 数字键 `5` (`0x40`)
     - **切换 UI 主题**：数字键 `3` (`0x47`) 或 `6` (`0x43`)
